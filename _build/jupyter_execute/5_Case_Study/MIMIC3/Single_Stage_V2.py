...
# |                  |   NDE   | NIE     | TE     |
# |------------------|:------:|--------|--------|
# | Direct Estimator | -.1713 |  .0029 | -.1684 |
# | IPW              | -.2380 | 0      | -.2380 |
# | Robust           | -.1826 | -.0237 | -.2063 |
# 
...
# \end{align}
# 
# Using the code below, we evaluated two target polices (regimes). The first one is a fixed treatement regime that applies no treatment (Policy1), with an estimated value of .9525. Another is a fixed treatment regime that applies treatment all the time (Policy2), with an estimated value of .7543. Therefore, the treatment effect of Policy2 comparing to Policy1 is -.1982, implying that receiving IV input increase the mortality rate.
...
# 
# 1. We would recommend $A=0$ (IV_Input = 0) if $.0007*\textrm{Glucose}+.0008*\textrm{PaO2_FiO2}<.5530$
...
# | 56         | 0        |
# | 1          | 1        |
# The estimated value of the estimated optimal policy is **.9525**.
