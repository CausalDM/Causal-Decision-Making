...
# # Causal Discovery Learner - NOTEARS Algorithm
# 
# Causal discovery attracts more and more attention recently to infer causal structure from data and disentangles the complex relationship among variables. In the following, we detail a state-of-the-art causal discovery learner, NOTEARS (Zheng et al., 2018).
...
# \end{equation}
# where the parameterized functions $f_1$ and $f_2$ effectively perform (possibly nonlinear) transforms on $\epsilon$ and $Z$, respectively. Here, Model 1 is a special case of Model 2. 
# 
